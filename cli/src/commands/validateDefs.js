...
export async function run(): Promise<number> {
  ...
  validationErrors.forEach((errors, pkgNameVersion) => {
    console.log("validate-defs: Found some problems with %s:", pkgNameVersion);
  ...
  if (validationErrors.size === 0) {
    console.log(
      `validate-defs: All library definitions are named and structured ` +
      `correctedly. (Found ${localDefs.length})`
    );
    return 0;
  }
